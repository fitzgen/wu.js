(function (globals, UNDEF) {
    ...
    var createNextMethodFor = function (obj) {
        ...
        makeNextForArrayLikeObjs = function (list) {
            // Based on Scheme code by Matthias Felleisen from
            // http://blog.racket-lang.org/2007/07/callcc-and-self-modifying-code.html
            var controlState = function (ret) {
                for (var i = 0; i < list.length; i++) {
                    ret = callcc(function (resume) {
                        controlState = resume;
                        ret(list[i]);
                    });
                }
        ...
        switch (toObjProtoString(obj)) {
            ...
            case OBJECT_OBJECT_STR:
                if (isInstance(obj, wu.Iterator)) {
                    ...
                    return makeNextForArrayLikeObjs(pairs);
                }
            case OBJECT_STRING_STR:
                return makeNextForArrayLikeObjs(obj.split(""));
