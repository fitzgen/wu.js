(function (globals) {
    ...
    // maximum number of arguments to curry. If it is missing, we default to one
    // less than the number of explicit arguments the function has.
    wu.autoCurry = function autoCurry(fn, maxToCurry) {
        maxToCurry = maxToCurry || fn.length - 1;
        return function autoCurried() {
            if (arguments.length <= maxToCurry) {
                return wu.curry.apply(this, [fn].concat(toArray(arguments)));
            }
            else {
